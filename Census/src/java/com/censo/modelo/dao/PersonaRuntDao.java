...
public class PersonaRuntDao {
    ...
    public boolean adicionarPersonaRunt(Connection conex, PersonaRunt personaRunt) throws SQLException {
        ...
        String sql = "INSERT INTO PERSONAS_RUNT (TIPO_DOCUMENTO_ID, TIPO_DOCUMENTO,NUMERO_DOCUMENTO,"
                ...
                + "FECHA_VENCIMIENTO,CATEGORIA_ID,CATEGORIA,ESTADO_LICENCIA,FECHA_CONSULTA,FUENTE_FALLO) "
                + "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        ...
        try (PreparedStatement pst = conex.prepareStatement(sql)) {
            ...
            pst.setString(15, personaRunt.getFuenteFallo());
            pst.executeUpdate();
    ...
    public boolean modificarPersonaRunt(Connection conex, PersonaRunt personaRunt) throws SQLException {

        String sql = "UPDATE PERSONAS_RUNT SET FECHA_CONSULTA = ? "
                + "WHERE NUMERO_DOCUMENTO = ? AND TIPO_DOCUMENTO = ? AND FUENTE_FALLO = ? ";

        try (PreparedStatement pst = conex.prepareStatement(sql)) {
            pst.setString(1, personaRunt.getFechaConsulta());
            pst.setString(2, personaRunt.getNumeroDocumento());
            pst.setString(3, personaRunt.getTipoDocumento());
            pst.setString(4, personaRunt.getFuenteFallo());
            pst.executeUpdate();

            return true;
        } catch (SQLException e) {
            System.err.println("Error en modificarPersonaRunt: " + e);
        }
        return false;
    }
    
    public PersonaRunt ConsultarPersonaRuntByDocumento(Connection conex, int tipodocumento, String documento, String fuentefallo) throws SQLException {

        String sql = "SELECT * FROM PERSONAS_RUNT WHERE TIPO_DOCUMENTO_ID = ? AND NUMERO_DOCUMENTO = ? AND FUENTE_FALLO = ?";
        try (PreparedStatement pst = conex.prepareStatement(sql)) {
            pst.setInt(1, tipodocumento);
            pst.setString(2, documento);
            pst.setString(3, fuentefallo);
